...
contract VoterTest is BaseTest {
    ...
    function testKillGaugeWithRewards() public {
        ...
        uint256 reward = GOB.balanceOf(address(voter));
        assertGt(reward, 0);
        uint256 claimableBefore = voter.claimable(address(gauge));
        assertApproxEqRel(claimableBefore, reward, 1e6);

        assertEq(GOB.balanceOf(address(minter)), 0);

        voter.killGauge(address(gauge));
        assertEq(voter.claimable(address(gauge)), 0);
        // Minimal remains from rounding
        assertLt(GOB.balanceOf(address(voter)), 1e2); // check for dust
        assertEq(GOB.balanceOf(address(minter)), claimableBefore);

        // zero-out rewards from minter so in a new rebase, new GOB is minted
        vm.prank(address(minter));
        GOB.transfer(address(1), 9999999999999999999999999);
        ...
        assertLt(GOB.balanceOf(address(voter)), 1e2); // check for dust
        assertGt(GOB.balanceOf(address(minter)), 0);
    }
    ...
    function testCanStillDistributeAllWithKilledGauge() public {
        ...
        assertEq(GOB.balanceOf(address(gauge)), 0);
        // gauge2 receives distributions
        assertEq(GOB.balanceOf(address(gauge2)), 4999999999999999999999999);
    }
    ...
    function testNotifyRewardAmount() public {
        ...
        deal(address(GOB), address(minter), TOKEN_1);
        vm.prank(address(minter));
        GOB.approve(address(voter), TOKEN_1);

        uint256 minterPre = GOB.balanceOf(address(minter));
        uint256 voterPre = GOB.balanceOf(address(voter));

        vm.prank(address(minter));
        vm.expectEmit(true, false, false, true, address(voter));
        emit NotifyReward(address(minter), address(GOB), TOKEN_1);
        voter.notifyRewardAmount(TOKEN_1);

        uint256 minterPost = GOB.balanceOf(address(minter));
        uint256 voterPost = GOB.balanceOf(address(voter));
