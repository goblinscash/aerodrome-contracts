...
contract VotingEscrowTest is BaseTest {
    ...
    function testDepositForWithManagedNFT() public {
        ...
        assertEq(GOB.allowance(address(escrow), address(lockedManagedReward)), 0);
        ...
        uint256 pre = GOB.balanceOf(address(lockedManagedReward));
        ...
        emit NotifyReward(address(escrow), address(GOB), 604800, reward);
        vm.expectEmit(false, false, false, true, address(escrow));
        emit MetadataUpdate(mTokenId);
        escrow.depositFor(mTokenId, reward);
        uint256 post = GOB.balanceOf(address(lockedManagedReward));
    ...
    function testCreateLock() public {
        GOB.approve(address(escrow), 1e25);
    ...
    function testWithdraw() public {
        ...
        uint256 preBalance = GOB.balanceOf(address(owner));

        skipAndRoll(lockDuration);
        escrow.withdraw(1);

        uint256 postBalance = GOB.balanceOf(address(owner));
    ...
    function testCannotSplitOverflow() public {
        ...
        uint256 escrowBalance = GOB.balanceOf(address(escrow));
    ...
    function testCannotDelegateBySigWithInvalidNonce() public {
        ...
        deal(address(GOB), alice, TOKEN_100K);
