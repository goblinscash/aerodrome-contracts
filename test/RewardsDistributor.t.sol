...
contract RewardsDistributorTest is BaseTest {
    ...
    function testInitialize() public {
        ...
        assertEq(distributor.token(), address(GOB));
    ...
    function testClaimWithExpiredNFT() public {
        // test reward claims to expired NFTs are distributed as unlocked GOB
        // ts: 608402
        GOB.approve(address(escrow), TOKEN_1M);
        ...
        uint256 pre = GOB.balanceOf(address(owner));
        vm.expectEmit(true, true, true, true, address(distributor));
        emit Claimed(tokenId, 604800, 3628800, 15491459054552564388715110);
        distributor.claim(tokenId);
        uint256 post = GOB.balanceOf(address(owner));

        locked = escrow.locked(tokenId); // update locked value post claim
        assertEq(post - pre, rebase); // expired rebase distributed as unlocked GOB
    ...
    function testClaimRebaseWithManagedLocks() public {
        ...
        assertLt(GOB.balanceOf(address(distributor)), 100); // dust
    }

    function testClaimRebaseWithDepositManaged() public {
        minter.updatePeriod(); // does nothing
        vm.startPrank(address(owner2));
        GOB.approve(address(escrow), TOKEN_10M);
    ...
    function testClaimOnLockedEnd() public {
        ...
        // Rebase should deposit into veNFT one second before expiry
        uint256 balanceBefore = GOB.balanceOf(address(owner));
        distributor.claim(tokenId);
        assertGt(GOB.balanceOf(address(owner)), balanceBefore);
    }
}
