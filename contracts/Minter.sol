...
contract Minter is IMinter {
    ...
    /// @inheritdoc IMinter
    uint256 public constant WEEKLY_GROWTH = 10_300; // 1.03  10300/10000
    /// @inheritdoc IMinter
    ...
    /// @inheritdoc IMinter
    uint256 public constant MAXIMUM_TEAM_RATE = 1000;
    /// @inheritdoc IMinter
    ...
    function initialize(AirdropParams memory params) external {
        ...
        for (uint256 i = 0; i < _len; i++) {
            gob.mint(params.liquidWallets[i], params.liquidAmounts[i]);
    ...
    function calculateGrowth(uint256 _minted) public view returns (uint256 _growth) {
        ...
        uint256 _aeroTotal = gob.totalSupply();
    ...
    function updatePeriod() external returns (uint256 _period) {
        ...
        if (block.timestamp >= _period + WEEK) {
            ...
            uint256 _emission;
            uint256 _totalSupply = gob.totalSupply();
            ...
            uint256 _balanceOf = gob.balanceOf(address(this));
            if (_balanceOf < _required) {
                gob.mint(address(this), _required - _balanceOf);
            }

            gob.safeTransfer(address(team), _teamEmissions);
            gob.safeTransfer(address(rewardsDistributor), _growth);
            rewardsDistributor.checkpointToken(); // checkpoint token balance that was just minted in rewards distributor

            gob.safeApprove(address(voter), _emission);
            voter.notifyRewardAmount(_emission);

            emit Mint(msg.sender, _emission, gob.totalSupply(), _tail);
        }
    }
}
